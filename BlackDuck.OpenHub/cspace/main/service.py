...
class CSpaceService:
    ...
    class ActionManager:
        ...
        def registerAction( self, action, actionCallback, actionOrder ):
            logger.debug("Registering action: %s" % action)
        ...
        def execAction( self, actionDir, contactName ) :
            ...
            for info in self.actions :
                if info[4] == actionDir :
                    logger.info("Executing action %s on contact %s" % (actionDir, contactName))
                    info[3]( contactName )
                    return True
                else:
    ...
    def __init__(self, seedNodes, settings=None, reactor=None):
        ...
        # Manage actions
        self.actionManager = self.ActionManager()
        # Loads applets in subprocesses
        self.appletServer = AppletServer( self.session, self.actionManager,
                                          self.reactor )

        # Our contacts' statii
        self.statusProbe = self.ContactStatusChecker(self, reactor)
    ...
    def online(self):
        ...
        logger.debug("Going online as %s..." % profile.name)
...
def _parseCommandLine():
    ...
    oparser.set_default('dhtnode', "210.210.1.102")
